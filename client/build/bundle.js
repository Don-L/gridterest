...
/***/ function(module, exports, __webpack_require__) {
	...
	var Gridterest = React.createClass({
	  ...
	  twoAdjacentTiles: function twoAdjacentTiles(array) {
	    if (array.length != 2) {
	      return false;
	    } else if (adjacent(array[0], array[1])) {
	      return true;
	    }
	  },
	
	  threeAdjacentTiles: function threeAdjacentTiles(array) {
	    if (array.length != 3) {
	      return false;
	    } else if (!twoAdjacentTiles(array[0], array[1])) {
	      return false;
	    } else if (array[1] === adjacentBottom(array[0]) && array[2] === adjacentBottom(array[1]) || array[1] === adjacentRight(array[0]) && array[2] === adjacentRight(array[1])) {
	      return true;
	    } else return false;
	  },
	
	  fourAdjacentTiles: function fourAdjacentTiles(array) {
	    if (array.length != 4) {
	      return false;
	    } else if (!threeAdjacentTiles([array[0], array[1], array[2]])) {
	      return false;
	    } else if (array[2] === adjacentBottom(array[1]) && array[3] === adjacentBottom(array[2]) || array[2] === adjacentRight(array[1]) && array[3] === adjacentRight(array[2])) {
	      return true;
	    } else return false;
	  },
	
	  twoByTwoSquare: function twoByTwoSquare(array) {
	    if (array.length != 4) {
	      return false;
	    } else if (adjacent(array[0], array[1]) && adjacent(array[2], array[3]) && adjacentBottom(array[0]) === array[2]) {
	      return true;
	    } else return false;
	  },
	
	  validSelectionGroup: function validSelectionGroup(array) {
	    if (singleTile(array) || twoAdjacentTiles(array) || threeAdjacentTiles(array) || fourAdjacentTiles(array) || twoByTwoSquare(array)) {
	      return true;
	    } else return false;
	  }
